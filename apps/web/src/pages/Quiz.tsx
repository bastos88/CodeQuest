...
export function Quiz() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      await Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: gamificationQueryKeys.events,
        }),
        queryClient.invalidateQueries({
          predicate: ({ queryKey }) =>
  ...
  const progress =
    session.questions.length > 0 ? (index / session.questions.length) * 100 : 0;
      ...
      <Card className="mx-auto max-w-4xl overflow-hidden p-8">
        ...
        <div className="relative text-center">
          ...
          <p className="mt-3 text-base text-textSecondary">
            Resultado consolidado no backend, sem revelar respostas corretas
            durante a sessão.
          </p>
            ...
            <div className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] p-5">
              ...
              <div className="mt-3 flex items-center justify-between text-sm text-textSecondary">
                ...
                <span className="font-mono">
                  {result.gamification.totalXP} XP
                </span>
              </div>
              <ProgressBar
                className="mt-3"
                value={result.gamification.level.percentage}
              />
              <p className="mt-3 text-xs text-textMuted">
                Sequência atual: {result.gamification.streak.current} dia
                {result.gamification.streak.current === 1 ? '' : 's'}
              </p>
            </div>
            <div className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] p-5">
              <p className="text-sm font-semibold text-textPrimary">
              ...
              <p className="mt-3 text-sm text-textSecondary">
                {result.gamification.newAchievements.length} conquista(s) e{' '}
                {result.gamification.completedMissions.length} missão(ões)
                concluída(s).
              </p>
              {result.gamification.completedMissions.map((mission) => (
                <p key={mission.code} className="mt-2 text-xs text-success">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {result.gamification.newAchievements.map((achievement) => (
                  <div
                    ...
                    className="flex items-start gap-3 rounded-[1.25rem] border border-primary/20 bg-primary/[0.06] p-4"
                  >
        ...
        <div className="relative">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:min-w-[360px]">
              ...
              <SessionBadge
                label="Progresso"
                value={`${answers.length}/${session.questions.length}`}
              />
            </div>
          </div>
          ...
          <Card ref={questionCardRef} className="scroll-mt-24 p-6">
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-3 text-3xl font-bold leading-tight tracking-[-0.04em] text-textPrimary">
                  {question?.prompt}
                </h3>
              </div>
              <div className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] px-4 py-3 text-right">
                <div className="font-mono text-sm text-textSecondary">
                  {index + 1}/{session.questions.length}
                </div>
                <div className="mt-1 text-xs uppercase tracking-[0.18em] text-textMuted">
                  questão
                </div>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <Circle
                        className={`h-5 w-5 shrink-0 ${selected ? 'text-primary' : 'text-textMuted'}`}
                      />
            ...
            <div className="mt-4 rounded-[1rem] border border-white/8 bg-white/[0.03] px-4 py-3 text-sm text-textSecondary">
              Sua resposta fica registrada, mas a validação acontece somente
              após a submissão final.
            </div>
        ...
        <aside className="space-y-6">
          <Card className="p-6">
            ...
            <div className="mt-5 space-y-3">
              ...
              <SidebarRow
                label="Restantes"
                value={`${session.questions.length - answers.length}`}
              />
              <SidebarRow
          ...
          <Card className="p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-textSecondary">
              Nenhuma resposta correta é enviada antes da submissão. O frontend
              só registra escolhas e tempos de resposta.
            </p>
          </Card>
        </aside>
      </div>
