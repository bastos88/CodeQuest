...
export function Dashboard() {
  ...
  if (profileQuery.isLoading) {
    return (
      <Card className="p-6 text-sm text-textSecondary">
    ...
    <div className="space-y-6">
          ...
          <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <Badge tone="success">
                  {gamification?.streak.current ?? profile.stats.streakDays}{' '}
                  dias de sequência
                </Badge>
              </div>
            </div>
            ...
            <div className="w-full max-w-md rounded-[1.25rem] border border-white/8 bg-white/[0.035] p-5 shadow-soft">
              ...
              <p className="mt-3 text-sm text-textSecondary">
                {Math.max(0, profile.level.nextLevelXP - profile.xp)} XP até o
                próximo nível
              </p>
            </div>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatTile
          ...
          value={`${profile.stats.streakDays} dias`}
          tone="success"
        />
        <StatTile
          icon={Code2}
        ...
        <Card className="flex flex-col p-6">
          <div className="flex-1">
            ...
            <p className="mt-2 text-sm leading-6 text-textSecondary">
              Escolha categorias, dificuldade e quantidade de perguntas na
        ...
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-textSecondary">
              Progresso válido até ao fim do dia UTC.
            </p>
          </div>
          <Badge tone="info">Hoje</Badge>
        </div>
        {missionsQuery.isLoading ? (
          <EmptyState>Carregando missões...</EmptyState>
        ) : null}
        {missionsQuery.isError ? (
          <EmptyState>Não foi possível carregar as missões diárias.</EmptyState>
        ) : null}
        {missionsQuery.data ? (
            ...
            {missionsQuery.data.missions.map((mission) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge tone={mission.completed ? 'success' : 'warning'}>
                    +{mission.xpReward} XP
                  </Badge>
                </div>
                <ProgressBar
      ...
      <Card className="p-6">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-textSecondary">
              Marcos desbloqueados e próximos objetivos.
            </p>
          </div>
          <Badge tone="success">
            {unlockedAchievements}/{achievements.length} desbloqueadas
          </Badge>
        </div>

        {achievementsQuery.isLoading ? (
          <EmptyState>Carregando conquistas...</EmptyState>
        ) : null}
        {achievementsQuery.isError ? (
          <EmptyState>Não foi possível carregar as conquistas.</EmptyState>
        ) : null}
        {achievements.length ? (
          ...
          <EmptyState>
            Complete quizzes para começar a desbloquear conquistas.
          </EmptyState>
        ) : null}
      </Card>
    </div>
    ...
    <Card className="p-6">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-textSecondary">
            Domínio calculado pela precisão e pelo volume de prática.
          </p>
        </div>
      ...
      {isError ? (
        <EmptyState>
          Não foi possível carregar seu mapa de habilidades.
        </EmptyState>
      ) : null}
      {!isLoading && !isError && skills.length === 0 ? (
        <div className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] p-5">
          <p className="text-sm leading-6 text-textSecondary">
            Você ainda não possui dados de habilidade. Complete um quiz para
            começar a mapear sua evolução.
          </p>
          <Button
            className="mt-4"
            variant="app"
              ...
              <div className="mb-2 flex flex-wrap items-end justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-textMuted">
                    {skill.accuracy}% de precisão · {skill.correct}/
                    {skill.answered} respostas corretas
                    {skill.lastPlayedAt
                      ? ` · ${formatLastPlayed(skill.lastPlayedAt)}`
                      : ''}
                  </p>
                </div>
...
function RankingCard({
  ...
  players: Awaited<ReturnType<typeof getPublicRanking>>;
  isLoading: boolean;
  isError: boolean;
}) {
  return (
    <Card className="p-6">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-textSecondary">
            Ranking geral atualizado com o progresso dos jogadores.
          </p>
        </div>
      ...
      {isError ? (
        <EmptyState>Não foi possível carregar o ranking.</EmptyState>
      ) : null}
      {!isLoading && !isError && players.length === 0 ? (
        <EmptyState>Nenhum jogador no ranking ainda.</EmptyState>
      ) : null}
...
function Avatar({
  ...
  return (
    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-primary/12 text-xs font-bold text-primary">
...
function EmptyState({ children }: { children: string }) {
  return (
    <div className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] p-4 text-sm text-textSecondary">
      {children}
    </div>
  );
}
