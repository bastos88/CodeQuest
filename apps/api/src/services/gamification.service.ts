...
const MASTERY_CATEGORIES = {
  frontend: new Set(['html', 'css', 'javascript', 'typescript', 'react', 'frontend']),
  backend: new Set(['node', 'nodejs', 'apis-rest', 'api-rest', 'backend', 'security', 'architecture']),
  database: new Set(['sql', 'database', 'databases', 'postgresql', 'postgres', 'mongodb']),
} as const;
...
function utcDayEnd(value: Date) {
  return new Date(Date.UTC(value.getUTCFullYear(), value.getUTCMonth(), value.getUTCDate() + 1) - 1);
}

function calculateStreak(lastActivityAt: Date | null, currentStreak: number, now: Date) {
  if (!lastActivityAt) return { streak: 1, activityDayChanged: true };
  const differenceInDays = Math.round(
    (utcDayStart(now).getTime() - utcDayStart(lastActivityAt).getTime()) / 86_400_000,
  );
  if (differenceInDays <= 0) return { streak: Math.max(1, currentStreak), activityDayChanged: false };
  if (differenceInDays === 1) return { streak: Math.max(1, currentStreak) + 1, activityDayChanged: true };
...
async function masteryMetrics(tx: Transaction, userId: string) {
  const answers = await tx.quizAnswer.findMany({
    ...
    select: {
      isCorrect: true,
      question: { select: { category: { select: { slug: true } } } },
    },
  });
  const totals = {
  ...
  return Object.fromEntries(
    Object.entries(totals).map(([key, value]) => {
      ...
      return [key, Math.min(100, Math.round(accuracy * 0.75 + Math.min(value.answered, 40) * 0.5))];
    }),
  ) as UserMetrics['mastery'];
}

function achievementProgress(code: string, metrics: UserMetrics, context: AchievementContext = {}) {
  const progress = (current: number, target: number) => ({ current: Math.min(current, target), target });
  switch (code) {
    case 'FIRST_QUIZ': return progress(metrics.quizzesCompleted, 1);
    case 'QUIZ_MASTER_10': return progress(metrics.quizzesCompleted, 10);
    case 'QUIZ_MASTER_50': return progress(metrics.quizzesCompleted, 50);
    case 'PERFECT_QUIZ': return progress(context.perfectQuiz ? 1 : 0, 1);
    case 'STREAK_3': return progress(metrics.streakDays, 3);
    case 'STREAK_7': return progress(metrics.streakDays, 7);
    case 'STREAK_30': return progress(metrics.streakDays, 30);
    case 'FRONTEND_MASTER': return progress(metrics.mastery.frontend, 80);
    case 'BACKEND_MASTER': return progress(metrics.mastery.backend, 80);
    case 'DATABASE_MASTER': return progress(metrics.mastery.database, 80);
    case 'CONTRIBUTOR': return progress(context.contributionApproved ? 1 : 0, 1);
    case 'ARENA_FIRST_WIN': return progress(context.arenaWin ? 1 : 0, 1);
    default: return progress(0, 1);
  }
}
...
async function unlockEligibleAchievements(
  ...
  for (const achievement of achievements) {
    ...
    await tx.gamificationEvent.create({
      data: {
        userId,
        type: achievement.category === 'mastery' ? 'CATEGORY_MASTERY' : 'ACHIEVEMENT_UNLOCKED',
    ...
    xpReward += achievement.xpBonus;
    newAchievements.push({ ...publicAchievement(achievement), unlockedAt: now.toISOString() });
  }
...
async function ensureDailyMissions(tx: Transaction, userId: string, now: Date) {
  ...
  for (const mission of missions) {
    await tx.userMission.upsert({
      where: { userId_missionId_expiresAt: { userId, missionId: mission.id, expiresAt } },
      update: {},
...
async function advanceDailyMissions(
  ...
  for (const mission of missions) {
    ...
    const increment = mission.key === 'COMPLETE_1_QUIZ'
      ? increments.quizzes
    ...
    if (didComplete) {
      ...
      completed.push({ code: mission.key, title: mission.title, xpReward: mission.xpReward });
      await tx.gamificationEvent.create({
        data: {
          userId,
...
export async function applyQuizGamification(
  ...
  const guarded = await tx.quizResult.updateMany({
    where: { id: input.quizResultId, userId: input.userId, gamificationAppliedAt: null },
    data: { gamificationAppliedAt: now },
  });
  if (guarded.count !== 1) throw new HttpError(409, 'Gamification already applied to this quiz');
  ...
  const categoryIds = [...new Set(result.answers.map((answer) => answer.question.categoryId))];
  ...
  const previousCategoryIds = new Set(previouslyPlayed.map((answer) => answer.question.categoryId));
  const firstCategoryCount = categoryIds.filter((id) => !previousCategoryIds.has(id)).length;
  const baseXP = XP_RULES.quizCompleted + result.correctCount * XP_RULES.correctAnswer;
  const hardBonus = result.session.difficulty === 'HARD' ? XP_RULES.hardQuizBonus : 0;
  ...
  const streakBonus = streak.activityDayChanged
    ? Math.min(streak.streak * XP_RULES.streakPerDay, XP_RULES.maximumStreakBonus)
    : 0;
  const quizXP = baseXP + hardBonus + firstCategoryBonus + perfectBonus + streakBonus;
  ...
  if (perfectBonus > 0) {
    await tx.gamificationEvent.create({
      data: { userId: input.userId, type: 'QUIZ_PERFECT', xpChange: perfectBonus, metadata: { quizResultId: result.id }, createdAt: now },
    });
  }
  if (streakBonus > 0) {
    await tx.gamificationEvent.create({
      data: { userId: input.userId, type: 'STREAK_BONUS', xpChange: streakBonus, metadata: { streakDays: streak.streak }, createdAt: now },
    });
  }
  ...
  await tx.quizResult.update({ where: { id: result.id }, data: { xpEarned: xpGained } });
  return {
    xpGained,
    totalXP,
    level: levelPayload(totalXP),
    streak: { current: streak.streak, longest: Math.max(user.longestStreak, streak.streak) },
    newAchievements: achievements.newAchievements,
    completedMissions: missions.completed,
  };
}

async function currentMetrics(tx: Transaction, userId: string): Promise<UserMetrics> {
...
export async function applyContributionGamification(tx: Transaction, userId: string, questionId: string) {
  const now = new Date();
  await tx.gamificationEvent.create({
    data: { userId, type: 'CONTRIBUTION_APPROVED', xpChange: XP_RULES.approvedQuestion, metadata: { questionId }, createdAt: now },
  });
...
export async function applyArenaGamification(tx: Transaction, userId: string, matchId: string, won: boolean) {
  const now = new Date();
  await tx.gamificationEvent.create({
    data: { userId, type: won ? 'ARENA_WIN' : 'ARENA_LOSS', xpChange: 0, metadata: { matchId }, createdAt: now },
  });
  if (!won) return;
  const unlocked = await unlockEligibleAchievements(
    tx,
  ...
  if (unlocked.xpReward > 0) {
    await tx.user.update({ where: { id: userId }, data: { xp: { increment: unlocked.xpReward } } });
  }
}
...
export async function getGamificationSummary(userId: string) {
  const user = await prisma.user.findUniqueOrThrow({
    where: { id: userId },
    select: { xp: true, streakDays: true, longestStreak: true, quizzesCompleted: true, correctAnswers: true, totalAnswers: true },
  });
  return {
    totalXP: user.xp,
...
export async function getAchievements(userId: string) {
  return prisma.$transaction(async (tx) => {
    const [achievements, unlocked, metrics] = await Promise.all([
      tx.achievement.findMany({ where: { isActive: true }, orderBy: [{ category: 'asc' }, { name: 'asc' }] }),
      tx.userAchievement.findMany({ where: { userId }, select: { achievementId: true, unlockedAt: true } }),
    ...
    return achievements.map((achievement) => {
      ...
      return {
        ...
        progress: unlockedAt ? { current: progress.target, target: progress.target } : progress,
      };
    });
  });
}
...
export async function getGamificationEvents(userId: string, page = 1, limit = 20) {
  ...
  const [items, total] = await prisma.$transaction([
    prisma.gamificationEvent.findMany({
      ...
      take: safeLimit,
      select: { id: true, type: true, xpChange: true, metadata: true, createdAt: true },
    }),
